...
class User(Document):
	...
	def on_trash(self):
		...
		frappe.db.delete("Route History", {"user": self.name})
...
def get_all_programs(arg=None):
    ...
    return [program.get("name") for program in programs]
...
@frappe.whitelist()
def update_user_mahasiswa():
	for u in frappe.db.sql(
			"""select s.* from tabStudent s
			INNER JOIN tabUser u ON u.email = s.student_email_id
			WHERE (s.user is null or s.user = '') and s.nim is not null and s.nim != ''""",
			as_dict=1
		):
		if u.student_email_id != None and u.student_email_id != '':
			user_doc = frappe.get_doc("User", u.student_email_id)
			user_doc.enabled = 1
			user_doc.language = "id"
			user_doc.role_profile_name = "Mahasiswa"
			user_doc.new_password = """ABC{0}""".format(u.nim)
			print(u.nim)
			user_doc.save()
			student_doc = frappe.get_doc("Student", u.name)
			student_doc.user = u.student_email_id
			student_doc.save()
			frappe.db.commit()
