...
frappe.ui.form.on('User', {
	...
	onload: function(frm) {
		...
		if(frm.can_edit_roles){
			if (!frm.roles_editor) {
				...
				frm.roles_editor = new frappe.RoleEditor(role_area, frm, 0, frm.is_new());
			...
			if(!frm.study_program_editor){
				const study_program_area = $('<div class="study-program-editor">')
					.appendTo(frm.fields_dict.study_program_html.wrapper);
				frm.study_program_editor = new frappe.StudyProgramEditor(study_program_area, frm, 0, frm.is_new());
			} else {
				frm.study_program_editor.show();
			}
		}
	...
	refresh: function(frm) {
		...
		frm.toggle_display(['sb1', 'sp1', 'sb3', 'modules_access', 'change_password'], false);

		if(!frm.is_new()) {
			if(has_access_to_edit_user()) {
				frm.toggle_display(['sb1', 'sp1', 'sb3', 'modules_access', 'change_password'], true);
			}
			...
			if(has_access_to_edit_user()) {
				frm.toggle_display(['sb1','sp1','change_password'], true); 
			}
		}
	...
	enabled: function(frm) {
		...
		if(!frm.is_new() && has_access_to_edit_user()) {
			frm.toggle_display(['sb1', 'sp1', 'sb3', 'modules_access', 'change_password'], doc.enabled);
...
function get_roles_for_editing_user() {
	return frappe.get_meta('User').permissions
		...
		.map(perm => perm.role).concat(['System Manager']);
}